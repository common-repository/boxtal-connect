		...
		if ( Misc_Util::should_display_parcel_point_link( $method ) ) {
			...
			if ( $points_response ) {
				...
				if ( null === $chosen_parcel_point ) {
					...
					$full_label .= '<br/><span class="' . Branding::$branding_short . '-parcel-client">' . __( 'Closest parcel point:', 'boxtal-connect' ) . ' <span class="' . Branding::$branding_short . '-parcel-name">' . $closest_parcel_point->name . '</span></span>';
					$tooltip     = $this->get_parcelpoint_address( $closest_parcel_point );
				} else {
					$full_label .= '<br/><span class="' . Branding::$branding_short . '-parcel-client">' . __( 'Your parcel point:', 'boxtal-connect' ) . ' <span class="' . Branding::$branding_short . '-parcel-name">' . $chosen_parcel_point->name . '</span></span>';
					$tooltip     = $this->get_parcelpoint_address( $chosen_parcel_point );
				}

				if ( null !== $tooltip ) {
					$full_label .= '<img class="' . Branding::$branding_short . '-tooltip-address" title="' . $tooltip . '" src="' . esc_url( WC()->plugin_url() ) . '/assets/images/help.png" height="16" width="16" />';
				}

				$full_label .= '<br/><span class="' . Branding::$branding_short . '-select-parcel" data-branding="' . Branding::$branding_short . '">' . __( 'Choose another', 'boxtal-connect' ) . '</span>';
			}
		}
		return $full_label;
	}
